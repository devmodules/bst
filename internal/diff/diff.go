...
func Diff(strA, strB string) string {
	aLines := strings.Split(strA, "\n")
	bLines := strings.Split(strB, "\n")
...
// DiffBytes returns a string containing a line-by-line unified diff of the input bytes.
func DiffBytes(a, b []byte) string {
	alines := bytes.Split(a, []byte("\n"))
	blines := bytes.Split(b, []byte("\n"))

	aStringLines := make([]string, len(alines))
	for i, line := range alines {
		aStringLines[i] = string(line)
	}
	bStringLines := make([]string, len(blines))
	for i, line := range blines {
		bStringLines[i] = string(line)
	}
	chunks := DiffChunks(aStringLines, bStringLines)
